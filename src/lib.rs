use crate::{
    effects::{add_effect, process_active_effects, remove_effect},
    prelude::*,
};
use bevy::prelude::*;
...
pub mod prelude {
    pub use crate::{
        calculation::{EffectCalculation, EffectMagnitude, StackingPolicy, StatScalingParams},
        effects::{ActiveEffects, ActiveTags, GameplayEffect},
        events::{
            AddEffect, AddEffectData, BoundsBreachedMetadata, EffectMetadata, OnBoundsBreached,
            OnEffectAdded, OnEffectRemoved, OnRepeatingEffectTriggered, RemoveEffect,
        },
        gameplay_stats::{GameplayStat, GameplayStats, StatTrait},
        stats,
        timing::{EffectDuration, RepeatingSmallTimer},
        GameplayEffectsPlugin, GameplayEffectsSystemSet, StackingBehaviors,
    };
}
...
impl<T: StatTrait> Plugin for GameplayEffectsPlugin<T> {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            Update,
            process_active_effects::<T>.in_set(GameplayEffectsSystemSet),
        );
...
mod tests {
    ...
    fn setup_entity<'a>(
        ...
        QueryState<(
            Entity,
            &'a GameplayStats<MyStats>,
            &'a ActiveEffects<MyStats>,
        )>,
    ) {
        let stats_component = GameplayStats::<MyStats>::new(|stat| match stat {
        ...
        let entity = app
            .world_mut()
            .spawn((stats_component, active_effects, active_tags))
            .id();
        let query = app
            .world_mut()
    ...
    fn test_periodic_effect() {
        ...
        app.world_mut().trigger(AddEffect(AddEffectData::new(
            ...
            GameplayEffect::new(
                ...
                EffectCalculation::Additive,
                EffectDuration::Repeating(RepeatingSmallTimer::new(1.0, 1.0), Some(10.0.into())),
            ),
            None,
        )));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
    ...
    fn test_persistent_with_duration() {
        ...
        app.world_mut()
            .trigger(AddEffect(AddEffectData::new(entity, buff.clone(), None)));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
        let health = stats.get(MyStats::Health).current_value;
        assert_eq!(health, 200.);
    ...
    fn test_persistent_removal() {
        ...
        app.world_mut()
            .trigger(AddEffect(AddEffectData::new(entity, buff1.clone(), None)));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
        let health = stats.get(MyStats::Health).current_value;
        assert_eq!(health, 200.);
        app.world_mut()
            .trigger(AddEffect(AddEffectData::new(entity, buff2.clone(), None)));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
        let health = stats.get(MyStats::Health).current_value;
        ...
        app.world_mut()
            .trigger(RemoveEffect(EffectMetadata::new(entity, buff1.tag, None)));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
        let health = stats.get(MyStats::Health).current_value;
        assert_eq!(health, 150.);

        app.world_mut()
            .trigger(RemoveEffect(EffectMetadata::new(entity, buff2.tag, None)));
        let (_, stats, _) = query.iter(app.world_mut()).next().unwrap();
        let health = stats.get(MyStats::Health).current_value;
        assert_eq!(health, 75.);
    }
    ...
    fn test_no_stacking() {
        ...
        app.world_mut()
            .trigger(AddEffect(AddEffectData::new(entity, effect.clone(), None)));
        app.world_mut()
            .resource_mut::<Time>()
            .advance_by(Duration::from_secs(1));
        app.update();
        ...
        app.world_mut()
            .trigger(AddEffect(AddEffectData::new(entity, effect, None)));
        app.world_mut()
            .resource_mut::<Time>()
            .advance_by(Duration::from_secs(1));
        app.update();
    ...
    fn test_tag_effect() {
        ...
        let mut query = app.world_mut().query::<(
            Entity,
            &GameplayStats<MyStats>,
            &ActiveEffects<MyStats>,
            &ActiveTags,
        )>();
